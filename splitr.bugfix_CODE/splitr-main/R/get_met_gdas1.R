...
get_met_gdas1 <- function(days,
  ...
  if (direction == "backward") {
    #AMK 2023/07/31: Added the "- lubridate::days(1)"
    min_date <- 
      (lubridate::as_date(days[1]) - (duration / 24) - lubridate::days(1)) %>%
      lubridate::floor_date(unit = "day")
  } else if (direction == "forward") {
    #AMK 2023/07/31: Added the "- lubridate::days(1)"
    min_date <- 
      (lubridate::as_date(days[1]) - lubridate::days(1)) %>%
      lubridate::floor_date(unit = "day")
  }
  ...
  if (direction == "backward") {
    #AMK 2023/07/31: Added the "+ lubridate::days(1)"
    max_date <- 
      (lubridate::as_date(days[length(days)]) + lubridate::days(1)) %>%
      lubridate::floor_date(unit = "day")
  } else if (direction == "forward") {
    #AMK 2023/07/31: Added the "+ lubridate::days(1)"
    max_date <- 
      (lubridate::as_date(days[length(days)]) + (duration / 24) + lubridate::days(1)) %>%
      lubridate::floor_date(unit = "day")
  }
