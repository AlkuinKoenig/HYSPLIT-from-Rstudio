...
######## USER INPUT ###############
job_workdir = paste0(here::here(),"/job_workdir") #this is the directory where the jobs will be "working". You can just define one
...
jobnumber = 8 #this gives the number of jobs, and thus the number of paralell processes. 
...
###################################################
control_datatable = expand.grid(lat = c(45.76583),
                                lon = 	c(6.864722),
                                starting_height=c(4807),
                                days_to_run = seq.Date(as.Date("2016-07-01"), as.Date("2016-07-24"), by = "1 day")) %>%
  dplyr::mutate(met_type = "gdas1", #the meteo input. Make sure that job_metdir points to the right direction!
                ...
                starting_hours = list(seq(0,23,6)))


job_config = set_config(kmsl = 1) #if kmsl = 0, interpret height input as "above ground level", if kmsl = 1, interpret it as "above sea level"
